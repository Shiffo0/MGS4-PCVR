...
int mgs4vr_menu_key(MGS4VR_MENU *m,unsigned key){
 ...
 if(key==VK_UP){m->row=(m->row+2)%3;return 1;}
 if(key==VK_DOWN){m->row=(m->row+1)%3;return 1;}
 if(key!=VK_LEFT && key!=VK_RIGHT && key!=VK_RETURN)return 0;
 if(m->row==0){if(!m->available)return 0;m->follow=!m->follow;m->recenter++;return 1;}
...
void *mgs4vr_menu_bitmap(const MGS4VR_MENU *m,int w,int h){
 ...
 font=CreateFontA(-h/13,0,0,0,FW_NORMAL,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH,"Segoe UI");
 old_font=SelectObject(dc,font);SetBkMode(dc,TRANSPARENT);SetTextColor(dc,RGB(235,241,250));
 for(row=0;row<6;++row){
  rect.left=w/24;rect.right=w-w/24;rect.top=h/16+row*h/7;rect.bottom=rect.top+h/9;
  if(row>=1 && row<=3 && m->row==row-1){brush=CreateSolidBrush(RGB(32,68,100));FillRect(dc,&rect,brush);DeleteObject(brush);}
  if(row==0)strcpy_s(text,sizeof(text),"MGS4-PCVR  v0.1.0");
  if(row==1)sprintf_s(text,sizeof(text),"Camera follows headset: %s",!m->available?"Unavailable":m->follow?"Yes":"No");
  ...
  if(row==4)strcpy_s(text,sizeof(text),"Up / Down: select    Enter: apply");
  if(row==5)strcpy_s(text,sizeof(text),"Home / Esc: close    Gamepad: play");
