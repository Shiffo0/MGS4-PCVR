...
int main(void){MGS4VR_MENU m;unsigned char *p;int i;
 ...
 mgs4vr_menu_key(&m,VK_DOWN);CHECK(m.row==3);
 CHECK(!mgs4vr_menu_key(&m,VK_RETURN) && !m.stereo);m.stereo_available=1;
 CHECK(mgs4vr_menu_key(&m,VK_RETURN) && m.stereo);
 mgs4vr_menu_key(&m,VK_DOWN);CHECK(m.row==0);CHECK(!mgs4vr_menu_key(&m,VK_RETURN));
 mgs4vr_menu_key(&m,VK_UP);CHECK(m.row==3);CHECK(mgs4vr_menu_key(&m,VK_RETURN) && !m.stereo);
